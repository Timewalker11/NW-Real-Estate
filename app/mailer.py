...
def send_contact_email(name: str, email: str, phone: Optional[str], body_text: str) -> None:
    if not GMAIL_USER or not GMAIL_APP_PASSWORD or not CONTACT_RECIPIENT:
        print("WARNING: GMAIL_USER/GMAIL_APP_PASSWORD/CONTACT_RECIPIENT not set; skipping contact email.")
        return

    message = EmailMessage()
    message["Subject"] = f"New website contact message from {name}"
    message["From"] = f"NW Real Estate Website <{GMAIL_USER}>"
